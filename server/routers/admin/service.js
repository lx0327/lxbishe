...
exports.login = async (req, res) => {
  ...
  if (result.length == 0) {
    ...
    let token = jwt.sign({ uid, username, jurisdiction }, PEIVATE_KEY, {
...
exports.changeadminstate = async (req, res) => {
  if (req.user.username == 'admin') {
    ...
    console.log(info)
    let sql =
      'update admin set isfk =?,isyh=?,isgl=?,issh=? ,user_state=? where user_id =?'
    const result = await query(sql, info)
    data = {
      state: s,
      data: {},
...
exports.changeuseruserstate = async (req, res) => {
  ...
  if (req.user.isyh == '1' || req.user.username == 'admin') {
    let info = [req.body.user_state, req.body.user_id]
    console.log(123)
    console.log(info)
...
exports.getcomment = async (req, res) => {
  ...
  let count = counts[0].count
  let sql =
    'select comment.comment_id ,comment.comment_content,comment.createtime,user.nickname,' +
...
exports.getreply = async (req, res) => {
  ...
  console.log(req.body)
  let sql =
    'select reply.reply_content ,reply.createtime,reply.touserid,reply.tousernickname,' +
...
exports.webgetwebactivitylist = async (req, res) => {
  ...
  let info = [pagesize, page]
  let sql =
    'select activity_id,activity_title,createtime from activity  limit ? offset ?'
  const result = await query(sql, info)
  if (result.length == 0) {
    data = {
      state: e,
...
;(exports.getuserlist = async (req, res) => {
  ...
  if (req.body.user !== '')
    sqlcounts = `${sqlcounts} and username like '%${req.body.user}%'`
  if (req.body.companystate !== '')
    sqlcounts = `${sqlcounts} and companystate ='${req.body.companystate * 1}'`
  if (req.body.realstate !== '')
    sqlcounts = `${sqlcounts} and realstate ='${req.body.realstate * 1}'`
  let infocounts = []
  ...
  if (req.body.user !== '')
    sql = `${sql} and username like '%${req.body.user}%'`
  if (req.body.companystate !== '')
    sql = `${sql} and companystate ='${req.body.companystate * 1}'`
  if (req.body.realstate !== '')
    sql = `${sql} and realstate ='${req.body.realstate * 1}'`
  sql = `${sql} limit ? offset ?`
  const result = await query(sql, info)
  data = {
    state: s,
  ...
  (exports.getadminlist = async (req, res) => {
    ...
    if (req.body.user !== '')
      sqlcounts = `${sqlcounts} and username like '%${req.body.user}%'`
    if (req.body.state !== '')
      sqlcounts = `${sqlcounts} and user_state ='${req.body.state}'`
    let infocounts = []
    const counts = await query(sqlcounts, infocounts)
    let count = counts[0].count
    let pagesize = req.body.pagesize * 1
    ...
    if (req.body.user !== '')
      sql = `${sql} and username like '%${req.body.user}%'`
    if (req.body.state !== '')
      sql = `${sql} and user_state ='${req.body.state}'`
    sql = `${sql} limit ? offset ?`
    const result = await query(sql, info)
    console.log(result)
    data = {
  ...
  (exports.changeuserstate = async (req, res) => {
    ...
    let info = [req.body.state, req.body.user_id]
    let sql = `update user set ${req.body.type}=? where user_id=?`
    const result = await query(sql, info)
    if (req.body.type === 'companystate' && req.body.state == 3) {
      const resultonly = await query('select * from company where user_id=?', [
        req.body.user_id,
      ])
...
exports.lablelist = async (req, res) => {
  ...
  if (req.body.lable_name !== '')
    sql = `${sql} where lable_name='${req.body.lable_name}'`
  const result = await query(sql, info)
  data = {
    state: s,
    data: result,
...
exports.changecarousel = async (req, res) => {
  ...
  let sql
  let info = [
    req.body.carousel_img,
    req.body.carousel_url,
    req.body.carousel_title,
    time,
    carousel_id,
  ]
  if (req.body.carousel_id == '')
    sql = `insert carousel  set carousel_img=?,carousel_url=?,carousel_title=?,carousel_createtime=?,carousel_id=?`
  else
    sql = `update carousel  set carousel_img=?,carousel_url=?,carousel_title=? where carousel_id='${req.body.carousel_id}'`
  const result = await query(sql, info)
  data = {
    state: s,
    data: result,
...
exports.changelable = async (req, res) => {
  let info = [req.body.lable, req.body.lable_id]
...
exports.changeadminuser = async (req, res) => {
  let info = [req.body.nickname, req.body.user_id]
  console.log(req.body)
...
exports.setannouncement = async (req, res) => {
  ...
  if (req.body.type === 'activity') {
    userlist = await query('select user_id from joins where content_id=?', [
      req.body.content_id,
    ])
...
exports.kefullist = async (req, res) => {
  ...
  if (req.body.id)
    sqlcounts = `${sqlcounts} and ${req.body.kefu_type}_id='${req.body.id}'`
  ...
  if (req.body.state)
    sql = `${sql} and ${req.body.kefu_type}_state=${req.body.state}`
