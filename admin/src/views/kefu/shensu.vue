        ...
        >
          <el-table-column prop="createtime" label="创建日期">
            <template slot-scope="scope">{{
              scope.row.shensu_createtime | dataFormat
            }}</template>
          </el-table-column>
          <el-table-column
            prop="shensu_user"
          ...
          <el-table-column prop="shensu_jubao_id" label="被封禁原因">
            <template slot-scope="scope">
              <el-button type="text" size="small" @click="tojubao(scope.row)"
                >查看被封禁原因</el-button
              >
            </template>
          </el-table-column>
          ...
          <el-table-column label="状态" prop="jubao_state">
            <template slot-scope="scope">
              <span style="color:#409eff" v-if="scope.row.shensu_state == 0"
                >未处理</span
              >
              <span style="color:#6cbb7a" v-if="scope.row.shensu_state == 1"
                >已处理</span
              >
              <span style="color:#f60c6c" v-if="scope.row.shensu_state == 2"
                >标记</span
              >
            </template>
          </el-table-column>
